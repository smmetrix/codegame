...
class ZeroDayApp:
    ...
    def _purchase_darknet_product(self, product: dict[str, object]) -> bool:
        ...
        if vpn_charge_value is not None and self.vpn_charges >= VPN_MAX_CHARGES:
        ...
        if vpn_charge_value is not None:
            ...
            result = f"VPN: +{added_charges} зарядов"
    ...
    def _update_hacker_attack_timer(self, delta_seconds: float) -> None:
        if not self.has_active_session or self.state != STATE_DESKTOP:
            return
    ...
    def _process_order(self, customer: str, decision: str) -> None:
        ...
        if decision == "accepted" and not self.arrested:
            ...
            order_context = {
                ...
                "difficulty": int(target_parameters["difficulty"]),
    ...
    def _handle_messenger_click(
        ...
        if selected == MESSENGER_FORUM_ID:
            return False
        order = self.messenger_active_orders.get(selected)
        if order is None or order.get("status") != "pending":
            return False
    ...
    def _draw_messenger_sidebar(
        ...
        for index, customer in enumerate(MESSENGER_CUSTOMERS):
            ...
            active_order = self.messenger_active_orders[nick]
            if active_order is not None and bool(active_order["is_story"]):
                pygame.draw.circle(
                    self.screen,
                    COLOR_ERROR,
                    self._point(
    ...
    def _draw_messenger_actions(
        ...
        order = self.messenger_active_orders.get(self.messenger_selected_chat)
        labels = {"accepted": "Принять", "rejected": "Отклонить"}
        if order is not None and bool(order.get("is_story")):
            labels = {
    ...
    def _draw_messenger_messages(
        ...
        right = self._messenger_right_rect(window)
        selected = self.messenger_selected_chat
        active_order = self.messenger_active_orders.get(selected)
        has_pending_order = (
        ...
        for message in self.messenger_messages[selected]:
            lines = self._wrap_text(str(message["text"]), font, text_width_pixels)
            timer_text = ""
            if (
                active_order is not None
                ...
                timer_text = self._format_deadline(float(active_order["deadline_at"]))
            height = max(
        ...
        for index, layout in enumerate(selected_layouts):
            ...
            pygame.draw.rect(
                self.screen,
                COLOR_MESSAGE_OUTGOING if outgoing else COLOR_MESSAGE_INCOMING,
        ...
        if has_pending_order:
            ...
            self._draw_text(
                "Кликните по чату ещё раз, чтобы получить новый заказ",
                (
                    right.centerx,
    ...
    def _draw_darknet_content(
        ...
        for slot, product in enumerate(self._darknet_visible_products()):
            ...
            has_funds = self.btc_balance + 1e-12 >= float(product["price"])
            vpn_at_capacity = (
